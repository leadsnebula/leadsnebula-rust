...
-- Backfill only when legacy tables exist (safe for test DBs with minimal schema)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pings') THEN
    ...
    INSERT INTO ping_payloads (id, ping_id, lead_id, payload, created_at, updated_at)
    SELECT gen_random_uuid(), p.id::text, p.lead_id, p.buyer_response, now(), now()
    FROM pings p
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'posts') THEN
    ...
    FROM posts pos
    WHERE pp.post_id = pos.id::text
      AND (pp.payload IS NULL)
      AND pos.buyer_response IS NOT NULL;

    -- insert post_payloads rows for posts that have buyer_response but no payload row
    INSERT INTO post_payloads (id, post_id, lead_id, payload, created_at, updated_at)
    SELECT gen_random_uuid(), pos.id::text, pos.lead_id, pos.buyer_response, now(), now()
    FROM posts pos
